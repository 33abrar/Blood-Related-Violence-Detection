...
Licensed under the MIST License (see LICENSE for details)
Written by Md. Mustaqim Abrar
"""
...
class MaskRCNN():
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            ...
            molded_image = mold_image(molded_image, self.config)        ###mold_image how??? [4]
            # Build image_meta
            image_meta = compose_image_meta(
                0, image.shape, molded_image.shape, window, scale,
                np.zeros([self.config.NUM_CLASSES], dtype=np.int32))        ###compose_image_meta??? [5]
            # Append
    ...
    def detect(self, images, verbose=0):
        ...
        molded_images, image_metas, windows = self.mold_inputs(images)      ###mold_inputs how??? [1]
        ...
        # Anchors
        anchors = self.get_anchors(image_shape)     ###get_anchors how??? [2]
        # Duplicate across the batch dimension because Keras requires it
        # TODO: can this be optimized to avoid duplicating the anchors?
        ...
        for i, image in enumerate(images):
            final_rois, final_class_ids, final_scores, final_masks =\
                self.unmold_detections(detections[i], mrcnn_mask[i],
                                       ...
                                       windows[i])      ###unmold_detections how??? [3]
            results.append({
                "rois": final_rois,
